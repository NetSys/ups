...
proc create-topology {topofoldername} {
    ...
    for {set i 0} {$i < $nlinks} {incr i} {
       ...
       if { $n2 < $ncores } {
          $ns duplex-link $nodes($n1) $nodes($n2) $bw $delay finePriorities
          $ns queue-limit $nodes($n1) $nodes($n2) $opt(maxq)
          $ns queue-limit $nodes($n2) $nodes($n1) $opt(maxq)

       } else {
          $ns simplex-link $nodes($n1) $nodes($n2) $bw $delay finePriorities
...
if { [info exists opt(control_packets)] } {
  Queue/finePriorities set control_packets_ $opt(control_packets)
}
